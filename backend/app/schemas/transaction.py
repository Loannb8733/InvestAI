...
class TransactionBase(BaseModel):
    ...
    fee: Money = Field(default=Decimal("0"), ge=0)
...
class TransactionUpdate(BaseModel):
    ...
    quantity: Optional[Money] = Field(None, gt=0)
    price: Optional[Money] = Field(None, ge=0)
    fee: Optional[Money] = Field(None, ge=0)
...
class TransactionResponse(BaseModel):
    ...
    asset_id: UUID
    transaction_type: TransactionType
    quantity: Money = Field(..., ge=0)  # Allow 0 for existing data
    price: Money = Field(..., ge=0)
    fee: Money = Field(default=Decimal("0"), ge=0)
    fee_currency: Optional[str] = Field(None, max_length=10)
    currency: str = Field(default="EUR", max_length=10)
