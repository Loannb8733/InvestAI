...
| **NEW-65** | 🟠 | **Changer son mot de passe ne coupe aucune session.** La révocation se fait **jeton par jeton**, à la déconnexion, via une liste de blocage Redis indexée par `jti`. Rien ne rattache un jeton déjà émis au mot de passe qui l'a produit : après un changement ou une réinitialisation, tous les jetons antérieurs restent valables — un jeton d'accès jusqu'à quinze minutes, un **jeton de rafraîchissement jusqu'à sept jours**. Or réinitialiser son mot de passe est précisément le geste par lequel on reprend la main sur un compte compromis : l'attaquant garde l'accès une semaine. | 🕓 **en attente d'arbitrage.** Le correctif habituel — un `tokens_valid_from` sur l'utilisateur, posé à chaque changement et comparé au `iat` du jeton dans `get_current_user` — est une migration et une vérification, mais il touche l'authentification de toute l'application : un défaut y couperait tout accès. Épinglé par deux tests qui montrent le jeton survivre. |
